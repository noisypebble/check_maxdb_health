...
sub new {
  ...
  if ($self->connect(%params)) {
    DBD::MaxDB::Cluster::add_cluster($self);
    $self->init(%params);
  }
  return $self;
}
...
sub init {
  ...
  if ($self->{show}) {
    ...
    foreach (split /\n/, $self->{show}) {
      if (/\[(\w+)\((\w+)\)\]\s+(\d+) node/) {
        ...
        push(@{$self->{nodes}}, DBD::MaxDB::Cluster::Node->new(
            type => $type,
            id => $1,
            status => $2,
        ));
  ...
  if ($params{mode} =~ /^cluster::ndbdrunning/) {
    foreach my $node (@{$self->{nodes}}) {
      ...
      $node->{type} eq "API" && $node->{status} eq "running" && $self->{maxdbd_nodes}++;
    }
  } else {
...
sub nagios {
  ...
  $self->add_perfdata(sprintf "ndbd_nodes=%d ndb_mgmd_nodes=%d maxdbd_nodes=%d",
      $self->{ndbd_nodes}, $self->{ndb_mgmd_nodes}, $self->{maxdbd_nodes});
}
...
sub DESTROY {
  ...
  if (ref($self) eq "DBD::MaxDB::Cluster") {
  }
...
sub requires_version {
  my $self = shift;
  my $version = shift;
  my @instances = DBD::MaxDB::Cluster::return_clusters();
...
package DBD::MaxDB::Cluster::Node;

use strict;

our @ISA = qw(DBD::MaxDB::Cluster);
...
sub new {
  ...
  if ($params{type} eq "NDB") {
    bless $self, "DBD::MaxDB::Cluster::Node::NDB";
    $self->init(%params);
  }
  return $self;
}
...
package DBD::MaxDB::Cluster::Node::NDB;

use strict;

our @ISA = qw(DBD::MaxDB::Cluster::Node);
