package DBD::MaxDB::Server::Instance;
...
sub init {
  ...
  if ($params{mode} =~ /server::instance::connectedthreads/) {
    ...
    if (DBD::MaxDB::Server::return_first_server()->version_is_minimum("5.1")) {
    ...
    if (DBD::MaxDB::Server::return_first_server()->version_is_minimum("5.1.3")) {
    ...
    $self->{fragmented} = [];
    #http://www.electrictoolbox.com/optimize-tables-maxdb-php/
    ...
    $self->{engine_myisam} = DBD::MaxDB::Server::Instance::MyISAM->new(
        %params
    );
  } elsif ($params{mode} =~ /server::instance::innodb/) {
    $self->{engine_innodb} = DBD::MaxDB::Server::Instance::Innodb->new(
        %params
    );
  } elsif ($params{mode} =~ /server::instance::replication/) {
    $self->{replication} = DBD::MaxDB::Server::Instance::Replication->new(
        %params
    );
  }
}
...
sub nagios {
  ...
  if (! $self->{nagios_level}) {
    if ($params{mode} =~ /server::instance::connectedthreads/) {
      ...
      if ($self->{uptime} > 10800) { # MaxDB Bug #30599
        $self->add_nagios(
