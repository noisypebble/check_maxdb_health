...
sub new {
  ...
  if ($self->dbconnect(%params)) {
    ...
    DBD::MaxDB::Server::add_server($self);
    $self->init(%params);
  }
  return $self;
}
...
sub init {
  ...
  if ($params{mode} =~ /^server::instance/) {
    $self->{instance} = DBD::MaxDB::Server::Instance->new(%params);
    ...
    foreach my $libpath (split(":", $DBD::MaxDB::Server::my_modules_dyn_dir)) {
      foreach my $extmod (glob $libpath."/CheckMaxDBHealth*.pm") {
        eval {
    ...
    bless $obj, "My$class";
    $self->{my} = $obj;
    if ($self->{my}->isa("DBD::MaxDB::Server")) {
      ...
      $self->add_nagios_unknown(
          sprintf "Class %s is not a subclass of DBD::MaxDB::Server%s", 
              ref($self->{my}),
  ...
  eval {
    if ($self->{handle}->fetchrow_array(q{
      ...
      my @dbthresholds = $self->{handle}->fetchall_array(q{
          SELECT * FROM check_maxdb_health_thresholds
      });
...
sub dbconnect {
  ...
  $self->{handle} = DBD::MaxDB::Server::Connection->new(%params);
...
sub trace {
  my $self = shift;
  my $format = shift;
  $self->{trace} = -f "/tmp/check_maxdb_health.trace" ? 1 : 0;
  if ($self->{verbose}) {
  ...
  if ($self->{trace}) {
    ...
    $logfh->autoflush(1);
    if ($logfh->open("/tmp/check_maxdb_health.trace", "a")) {
...
sub DESTROY {
  ...
  if (ref($self) eq "DBD::MaxDB::Server") {
  }
...
sub requires_version {
  my $self = shift;
  my $version = shift;
  my @instances = DBD::MaxDB::Server::return_servers();
  my $instversion = $instances[0]->{version};
  if (! $self->version_is_minimum($version)) {
    $self->add_nagios($ERRORS{UNKNOWN}, 
        sprintf "not implemented/possible for MaxDB release %s", $instversion);
  }
}
...
sub system_vartmpdir {
  ...
  if ($^O =~ /MSWin/) {
    ...
    return "/var/tmp/check_maxdb_health";
  }
}
...
package DBD::MaxDB::Server::Connection;

use strict;

our @ISA = qw(DBD::MaxDB::Server);
...
sub new {
  ...
  if ($params{method} eq "dbi") {
    bless $self, "DBD::MaxDB::Server::Connection::Dbi";
  } elsif ($params{method} eq "maxdb") {
    bless $self, "DBD::MaxDB::Server::Connection::Mysql";
  } elsif ($params{method} eq "sqlrelay") {
    bless $self, "DBD::MaxDB::Server::Connection::Sqlrelay";
  }
  $self->init(%params);
  return $self;
}
...
package DBD::MaxDB::Server::Connection::Dbi;

use strict;
use Net::Ping;

our @ISA = qw(DBD::MaxDB::Server::Connection);
...
sub init {
  ...
  if ($self->{mode} =~ /^server::tnsping/) {
    ...
    $self->{dsn} .= sprintf ";maxdb_socket=%s", $self->{socket} 
        if $self->{socket};
  }
...
sub selectrow_hashref {
  ...
  if (-f "/tmp/check_maxdb_health_simulation/".$self->{mode}) {
    my $simulation = do { local (@ARGV, $/) =
        "/tmp/check_maxdb_health_simulation/".$self->{mode}; <> };
...
package DBD::MaxDB::Server::Connection::Mysql;
...
sub init {
  ...
  if (! exists $self->{errstr}) {
    eval {
      my $maxdb = '/'.'usr'.'/'.'bin'.'/'.'maxdb';
      if (! -x $maxdb) {
        die "nomaxdb\n";
      }
      if ($self->{loginstring} eq "traditional") {
        $self->{sqlplus} = sprintf "%s ", $maxdb;
...
sub selectrow_hashref {
  ...
  if ($sql =~ /^\s*SHOW/) {
    $sql .= '\G'; # http://dev.maxdb.com/doc/refman/5.1/de/show-slave-status.html
  }
...
package DBD::MaxDB::Server::Connection::Sqlrelay;
