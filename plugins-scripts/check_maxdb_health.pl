...
use File::Basename;
use lib dirname($0);
use Nagios::DBD::MaxDB::Server;
use Nagios::DBD::MaxDB::Cluster;
...
$PROGNAME = "check_maxdb_health";
$REVISION = '$Revision: #PACKAGE_VERSION# $';
$CONTACT = 'dlittle@toyatech.net';
$TIMEOUT = 60;
...
sub print_usage () {
  ...
  Options:
    ...
    --port
       the database's port. (default: 7210)
    --socket
       the database's unix socket.
    --username
       the maxdb db user
    --password
       the maxdb db user's password
    --database
...
EOUS
  ...
  For your convenience you can call check_maxdb_health with the --mode encode
  option and it will encode the standard input.

  You can find the full documentation at 
  http://www.toyatech.net/opensource/nagios/check-maxdb-health
...
sub print_help () {
  print "Copyright (c) 2012 Dusty Little\n\n";
  print "\n";
  print "  Check various parameters of MaxDB databases \n";
  print "\n";
  print_usage();
  support();
}
...
sub support () {
  my $support='Send email to dlittle@toyatech.net if you have questions\nregarding use of this software. \nPlease include version information with all correspondence (when possible,\nuse output from the --version option of the plugin itself).\n';
  $support =~ s/@/\@/g;
  $support =~ s/\\n/\n/g;
  print $support;
}
...
if (exists $commandline{verbose}) {
  $DBD::MaxDB::Server::verbose = exists $commandline{verbose};
}

if (exists $commandline{scream}) {
#  $DBD::MaxDB::Server::hysterical = exists $commandline{scream};
}

if (exists $commandline{method}) {
  # snmp or maxdb cmdline
} else {
...
if (exists $commandline{'with-mymodules-dyn-dir'}) {
  $DBD::MaxDB::Server::my_modules_dyn_dir = $commandline{'with-mymodules-dyn-dir'};
} else {
  $DBD::MaxDB::Server::my_modules_dyn_dir = '#MYMODULES_DYN_DIR#';
}
  ...
  foreach my $important_env qw(LD_LIBRARY_PATH SHLIB_PATH 
    ...
    if ($ENV{$important_env} && ! scalar(grep { /^$important_env=/ } 
      ...
      printf STDERR "add important --environment %s=%s\n", 
          $important_env, $ENV{$important_env} if $DBD::MaxDB::Server::verbose;
    }
  }
}
...
if (! exists $commandline{statefilesdir}) {
  if (exists $ENV{OMD_ROOT}) {
    $commandline{statefilesdir} = $ENV{OMD_ROOT}."/var/tmp/check_maxdb_health";
  } else {
...
my %params = (
    ...
    method => $commandline{method} ||
        $ENV{NAGIOS__SERVICEMAXDB_METH} ||
        $ENV{NAGIOS__HOSTMAXDB_METH} || 'dbi',
    hostname => $commandline{hostname} || 
        $ENV{NAGIOS__SERVICEMAXDB_HOST} ||
        $ENV{NAGIOS__HOSTMAXDB_HOST} || 'localhost',
    database => $commandline{database} || 
        $ENV{NAGIOS__SERVICEMAXDB_DATABASE} ||
        $ENV{NAGIOS__HOSTMAXDB_DATABASE} || 'information_schema',
    port => $commandline{port}  || (($commandline{mode} =~ /^cluster/) ?
        ($ENV{NAGIOS__SERVICENDBMGM_PORT} || $ENV{NAGIOS__HOSTNDBMGM_PORT} || 1186) :
        ($ENV{NAGIOS__SERVICEMAXDB_PORT} || $ENV{NAGIOS__HOSTMAXDB_PORT} || 3306)),
    socket => $commandline{socket}  || 
        $ENV{NAGIOS__SERVICEMAXDB_SOCKET} ||
        $ENV{NAGIOS__HOSTMAXDB_SOCKET},
    username => $commandline{username} || 
        $ENV{NAGIOS__SERVICEMAXDB_USER} ||
        $ENV{NAGIOS__HOSTMAXDB_USER},
    password => $commandline{password} || 
        $ENV{NAGIOS__SERVICEMAXDB_PASS} ||
        $ENV{NAGIOS__HOSTMAXDB_PASS},
...
if ($params{mode} =~ /^(server|my)/) {
  $server = DBD::MaxDB::Server->new(%params);
...
} #TODO: don't know is maxdb has cluster elsif ($params{mode} =~ /^cluster/) {
#  $cluster = DBD::MaxDB::Cluster->new(%params);
