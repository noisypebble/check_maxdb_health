#! /usr/bin/perl -w -I ..
#
# MaxDB Database Server Tests via check_maxdb_healthdb
#
#
...
# Check with:
#  maxdb -u$user -p$password -h$host $db
...
plan skip_all => "check_maxdb_health not compiled" unless (-x "./check_maxdb_health");

plan tests => 51;

my $bad_login_output = '/Access denied for user /';
my $maxdbserver = getTestParameter( 
		"NP_MAXDB_SERVER", 
		"A MaxDB Server with no slaves setup"
		);
my $maxdb_login_details = getTestParameter( 
		"MAXDB_LOGIN_DETAILS", 
...
my $with_slave = getTestParameter( 
		"NP_MAXDB_WITH_SLAVE", 
		"MaxDB server with slaves setup"
		);
my $with_slave_login = getTestParameter( 
		"NP_MAXDB_WITH_SLAVE_LOGIN", 
...
my $result;
SKIP: {
	$result = NPTest->testCmd("./check_maxdb_health -V");
	cmp_ok( $result->return_code, '==', 0, "expected result");
	like( $result->output, "/check_maxdb_health \\(\\d+\\.\\d+\\)/", "Expected message");

	$result = NPTest->testCmd("./check_maxdb_health --help");
	cmp_ok( $result->return_code, '==', 0, "expected result");
...
SKIP: {
	$result = NPTest->testCmd("./check_maxdb_health -H $maxdbserver -m connection-time -u dummy -pdummy");
	...
	$result = NPTest->testCmd("./check_maxdb_health");
	cmp_ok( $result->return_code, "==", 3, "No mode defined" );
	like( $result->output, "/Must specify a mode/", "Correct error message");

	$result = NPTest->testCmd("./check_maxdb_health -m connection-time -w 10 -c 30");
	...
	$result = NPTest->testCmd("./check_maxdb_health -m keycache-hitrate -w :10 -c 2");
	...
	$result = NPTest->testCmd("./check_maxdb_health -m qcache-hitrate -w :10 -c 2");
	cmp_ok( $result->return_code, "==", 2, "Connected" );
	like( $result->output, "/CRITICAL - Query Cache Hitrate at ([0-9\.]+)%|qcache_hitrate=([0-9\.]+)%;:10;2/", "Correct error message");

	$result = NPTest->testCmd("./check_maxdb_health -m qcache-hitrate -w :10 -c 2 -v 2>&1");
	cmp_ok( $result->return_code, "==", 2, "Connected" );
...
SKIP: {
	...
	my $slow_queries = 0;
        my $delta = 0;
	$result = NPTest->testCmd("./check_maxdb_health -m slow-queries -w :10 -c 2");
	sleep 1;
	$result = NPTest->testCmd("./check_maxdb_health -m slow-queries -w :10 -c 2 -v 2>&1");
...
SKIP: {
	# performance data
	$result = NPTest->testCmd("./check_maxdb_health -m slow-queries -w :11 -c :22 -v 2>&1");
	like( $result->output, "/slow_queries_rate=[0-9\.]+;:11;:22 slow_queries=[0-9]+;:11;:22/", "Correct error message");

	$result = NPTest->testCmd("./check_maxdb_health -m qcache-lowmem-prunes -w :11 -c :22 -v 2>&1");
	like( $result->output, "/lowmem_prunes_rate=[0-9\.]+;:11;:22 lowmem_prunes=[0-9]+;:11;:22/", "Correct error message");

	$result = NPTest->testCmd("./check_maxdb_health -m bufferpool-wait-free -w :11 -c :22 -v 2>&1");
	like( $result->output, "/bufferpool_free_waits_rate=[0-9\.]+;:11;:22 bufferpool_free_waits=[0-9]+;:11;:22/", "Correct error message");

	$result = NPTest->testCmd("./check_maxdb_health -m log-waits -w :11 -c :22 -v 2>&1");
...
SKIP: {
	skip "No maxdb server with slaves defined", 5 unless $with_slave;
	$result = NPTest->testCmd("./check_maxdb_health -H $with_slave $with_slave_login");
	cmp_ok( $result->return_code, '==', 0, "Login okay");

	$result = NPTest->testCmd("./check_maxdb_health -S -H $with_slave $with_slave_login");
	cmp_ok( $result->return_code, "==", 0, "Slaves okay" );

	$result = NPTest->testCmd("./check_maxdb_health -S -H $with_slave $with_slave_login -w 60");
